...
(define (measure-continuous-attribute attrib class examples)
  (let* ((func (cdr attrib))
    ...
    (define (loop list seen acc) 
      (cond ((null? list) acc)
	    ((member (car list) seen) (loop (cdr list) seen acc))
	    (else (let ((gains (candidate-gain (car list))))
		    (if (< (gain-val gains) (gain-val acc))
			(loop (cdr list) (cons (car list) seen) gains)
			(loop (cdr list) (cons (car list) seen) acc))))))
    (loop examples (list (car examples)) (candidate-gain (car examples))))) 
